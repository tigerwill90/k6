...
func newStoppedVUHandle(
	...
	returnVU func(lib.InitializedVU), config *BaseConfig, ss *lib.ScenarioState,
	logger *logrus.Entry,
) *vuHandle {
...
func (vh *vuHandle) start() (err error) {
	...
	case stopped, toHardStop: // we need to reactivate the VU and remake the context for it
		...
		vh.activeVU = vh.initVU.Activate(getVUActivationParams(vh.ctx, *vh.config, vh.returnVU, vh.scenarioState))
		close(vh.canStartIter)
